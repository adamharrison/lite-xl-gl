...
if [[ "$@" != *"-lfreetype"* ]]; then
  ...
  LLSRCS=" $LLSRCS lib/freetype/src/base/ftsystem.c lib/freetype/src/base/ftinit.c lib/freetype/src/base/ftdebug.c lib/freetype/src/base/ftbase.c \
    ...
    lib/freetype/src/gzip/ftgzip.c lib/freetype/src/base/ftbitmap.c lib/freetype/src/base/ftmm.c"
  [ ! -e "lib/prefix/lib/libfreetype2.a" ] && { $CC -c $LINK_FLAGS $COMPILE_FLAGS $LLSRCS -c && $AR rc lib/prefix/lib/libfreetype2.a *.o && rm *.o || exit -1; }
  LINK_FLAGS="$LINK_FLAGS -lfreetype2"
fi
...
if [[ "$@" != *"-lSDL"* ]]; then
  ...
  [ ! -e "lib/SDL/build" ] && { cd lib/SDL && mkdir build && cd build && cmake .. -G "Unix Makefiles" $CMAKE_DEFAULT_FLAGS \
    -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DSDL_INSTALL=ON -DSDL_INSTALL_DOCS=OFF -DSDL_DEPS_SHARED=ON \
		...
		-DSDL_DBUS=OFF -DSDL_IBUS=OFF -DSDL_AUDIO=OFF -DSDL_GPU=ON -DSDL_RPATH=OFF -DSDL_PIPEWIRE=OFF \
